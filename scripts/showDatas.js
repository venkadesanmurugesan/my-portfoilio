    ...
    docRef
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          for (let i = 0; i <  doc.data()['education_details'].length; i++) {
            educationForm.innerHTML += `<tr id="eduDataRow${i + 1}">
               ...
               <td>${ doc.data()['education_details'][i]["instituteName"]}</td>
                <td>${ doc.data()['education_details'][i]["instituteAddress"]}</td>
                <td>${ doc.data()['education_details'][i]["startYearAndMonth"]}</td>
                <td>${ doc.data()['education_details'][i]["endYearAndMonth"]}</td>
          ...
          let experienceForm = document.getElementById(
            "expDetailsResult"
          );
       ...
       <td class="d-flex justify-content-between">
        ...
        let contactForm = document.getElementById("contact_form").querySelectorAll('input');
